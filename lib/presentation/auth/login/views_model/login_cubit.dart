...
import 'package:movies_app/domain/use_cases/login/login_with_google_use_case.dart';
...
class LoginCubit extends Cubit<LoginState> {
        ...
        case Failure<void>():
          {
            ...
            emit(state.copyWith(loginStatus: const StateStatus.initial()));
            break;
          }
      }
    } else {
  ...
  Future<void> _loginWithGoogle() async {
    emit(state.copyWith(loginStatus: const StateStatus.loading()));
    final result = await _loginWithGoogleUseCase.invoke();
    switch (result) {
      case Success<void>():
        {
          emit(state.copyWith(loginStatus: const StateStatus.success(null)));
